...
void jass_dumpenv(LPJASS j) {
    ...
    if(zero->type->typeid!=jasstype_function) //locals alwasy bound to the target function
    {
        jass_dumpstack(j);
        printf("locals binding is not on a target function\n");
        assert(0);
    }
    fprintf(stdout, "\n[locals(0) %p]\n",zero);
