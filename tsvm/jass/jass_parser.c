...
static BOOL parse_body(LPPARSER p, LPTOKEN function) {
    ...
    if (func && (token = func(p))) {
        ...
        LPCSTR tk= peek_token(p);
        PARSER_THROW("error parsing function near %s %d, unexpected token `%s`", p->file, p->line,tk);
    }
    return true;
}
...
//    LPTOKEN token = alloc_token(TT_IDENTIFIER, p);
...
PARSER(parse_args) {
    ...
    while (!args || eat_token(p, ",")) {
        LPTOKEN arg = alloc_token(TT_VARDECL, p);
